    ...
    Array.prototype.slice.call(submitButtons).forEach(function (button) {
        button.addEventListener('click', function (event) {
            ...
            if (isValid) {
                ...
                $('.modal-body p').html('Are you sure you want to add the post titled <strong>' + title + '</strong>?');
                $('#add-modal').modal("show"); // Show the modal
            }
        }, false);
    });

    // Handle modal confirmation for adding post
    $('#modal-add-yes').click(function() {
        // Submit the form once the user confirms
        $('#addForm').submit(); // Trigger form submission
